...
def initial_mesh():
    ...
    # print(pointList)
    # print([np.arange(1,len(pointList)+1)])
    # print(pointList[-1])
...
def sim(mesh,iteration_step,dt,remesh=False, save=False):
    ...
    #     def __init__(self, vp, dvp, lmda, **kwargs):
    #         super().__init__(**kwargs)
    #         self.vp = vp 
    #         self.dvp=dvp
    #         self.lmda=lmda
    #     def eval(self, values, x):
    #         # values[1] = self.vp + self.dvp*sin(2*pi*x[0]/self.lmda)
    #         values[1] = self.vp + self.dvp*sin(2*np.pi*x[0]/self.lmda)
    #         values[0]=0.0 #the other direction is zero
    #     def value_shape(self):
    #         return(2,)
    class InflowExpression(UserExpression): #
        ...
        def eval(self, values, x):
            # values[0] = self.vp + self.dvp*sin(2*np.pi*x[0]/self.lmda)
            # values[1] = 0
    ...
    def fixed_boundary_problem(mesh, iteration_step):
        ...
        u0 = interpolate(Constant((0.0, 1.0)), V_CG2)
        rho0 = interpolate(Constant(rho_in), K_CG1)
        # p0 = interpolate(Constant(0), K_CG1)

        assigner = FunctionAssigner(W,[V_CG2,K_CG1])
        assigner.assign(U, [u0, rho0])    

        # ---------- Pressure & stress ----------
        p = chi * (rho - rho_target)
        # p = alpha * rho
        ...
        sigmaW = stress_function(w, 0*rho)  # test stress; no pressure part
        ...
        # j_in = InflowExpression(vp=vp, alpha=alpha, rho_in=rho_in, amp=amp, q=q, degree=2) # total inflow flux at bottom
        bc_bottom = DirichletBC(W.sub(1), rho_in, boundaries, 1)


        # F_rho = (
        #     + kd*rho*ws*dx
        #     - j_in*ws*ds_bottom
        #     # + D*dot(grad(rho), n_exp_cg2)*ws*ds_bottom
        #     - inner(rho*v, grad(ws))*dx
        #     + D*inner(grad(rho), grad(ws))*dx
        #     # + (dot(rho*v, n)) * ws * ds_top
        #     + (dot(rho*v, n) - D*dot(grad(rho), n)) * ws * ds_top
        # )
        F_rho = (
            + kd*rho*ws*dx
            # + D*dot(grad(rho), n_exp_cg2)*ws*ds_bottom
        ...
        # ---------- Solve ----------
        solve(F == 0, U,[bc_bottom])  # all BCs via periodicity + Nitsche + natural
    ...
    def harmonic_extension(mesh,U_stokes,dt,iteration_step):
        ...
        rho_harmonic = FunctionSpace(mesh, P2, constrained_domain=pbc)
        ...
        rho_stokes = Function(K_CG1, name='Dens')
        v_stokes.assign(interpolate(U_stokes.sub(0),V_harmonic))
        rho_stokes.assign(interpolate(U_stokes.sub(1),rho_harmonic))
        ...
        #     return assemble(inner(u,u)*ds_bound)
        
        vel_CG2 = Function(V_harmonic, name='Velocity')
        vel_CG2.assign(u)
        # errMeasure = sqrt(err(vel_CG2,ds_top)/params['CDLX_ND'])
        ...
        for coord in boundary_coords_top_sorted:
            ...
            rho_vals.append(rho_stokes((x,y)))


        errMeasure = np.max(np.array(rho_vals)*np.array(vy_vals))
        n_t = FacetNormal(mesh)
        # errMeasure = assemble(rho_stokes*inner(dot(Identity(2), v_stokes), n_t) * ds_top(degree=7))
        np.savetxt('errMeasure.txt', [errMeasure])  
        print("ERR MEASURE ", errMeasure)

        # v, rho = split(U_stokes)     
        # n_t = FacetNormal(mesh)
        # errMeasure = abs(float(assemble((rho*dot(v, n_t)) * ds_top)))
        # np.savetxt('errMeasure.txt', [errMeasure])
        # print("ERR MEASURE (top flux) ", errMeasure)
